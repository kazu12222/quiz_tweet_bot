...
(async ()=> {
    ...
    const [botsData,cnt,ansOpenDateValue] = await time.myFunc();
    for (let i = 0; i < cnt; i++) {
    ...
    ansOpenDate= (isNaN(structAnsDay)) ? structAnsDay.replace(/[^0-9/]|/g, '') : null;
    await twitterClient.v2.tweet(user+"さんから新規Quizご登録いただきました！\n一般解答公開日は"+ansOpenDate+"です。\n\n"+title+"("+createTime+")\n"+"#AAAAAA\n"+href);
    }
    else{
    await twitterClient.v2.tweet(user+"さんから新規Quizご登録いただきました！\n一般解答公開は公開済みです。\n\n"+title+"("+createTime+")\n"+"#AAAAAA\n"+href);  
    }
